...
from cfe.interpreter import CfeRuntimeError, execute, set_gateway
...
class TestDefineAgent:
    ...
    def test_define_agent_not_in_config(self):
        with pytest.raises(CfeRuntimeError, match="not defined in the agent"):
...
class TestTellHearSimple:
    ...
    def test_tell_undeclared_agent(self):
        gw, _ = _make_gateway()
        with pytest.raises(CfeRuntimeError, match="has not been declared"):
            _run("tell testbot text, hi.", gw=gw)

    def test_hear_without_tell(self):
        with pytest.raises(CfeRuntimeError, match="No pending response"):
...
class TestResponseFields:
    ...
    def test_field_of_non_response(self):
        with pytest.raises(CfeRuntimeError, match="not an agent response"):
            _run("""
            set x to 5.
