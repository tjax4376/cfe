"""
CFE v2 Type System – runtime value types and coercion rules.
...
def type_of(value: Any) -> CfeType:
    """Return the CFE type for a runtime Python value."""
    if value is None:
        return CfeType.NONE
    if isinstance(value, bool):
        return CfeType.TRUTH
    if isinstance(value, int):
        return CfeType.NUMBER
    if isinstance(value, float):
        return CfeType.DECIMAL
    if isinstance(value, str):
        return CfeType.TEXT
    raise TypeError(f"Unknown CFE value type: {type(value).__name__}")


def type_name(t: CfeType) -> str:
    """Human-readable name for display in error messages."""
    return {
        CfeType.NUMBER: "number",
        CfeType.DECIMAL: "decimal",
        CfeType.TEXT: "text",
        CfeType.TRUTH: "truth",
        CfeType.NONE: "none",
    }[t]
